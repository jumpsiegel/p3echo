...
def approve_app():
    ...
    router = Cond(
        [METHOD == MethodSignature("portal_transfer(byte[])byte[]"), receiveMessage()],
    )

    on_create = Seq( [
        App.globalPut(Bytes("coreid"), Btoi(Txn.application_args[0])),
        App.globalPut(Bytes("tokenid"), Btoi(Txn.application_args[1])),
        Return(Int(1))
    ])
...
if __name__ == "__main__":
    ...
    sender = Account.FromMnemonic(sys.argv[3])
